...
conceder por sí sola la capacidad de operar: mientras no haya un perfil activo, toda ruta que
requiera sesión SHALL rechazar la petición.

Es lo que hace que el PIN de adulto sea una frontera y no un adorno de la interfaz: sin esto, un niño
podría saltarse la rejilla llamando directamente al endpoint y aprobarse sus propias tareas.
...
- **WHEN** se llama directamente a una operación reservada al padre con solo la sesión de cuenta
...
cuando el perfil activo es el del padre o cuando aún no se ha elegido ninguno.
